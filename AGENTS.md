...
**PATH Note**: If `which bn` shows a different path (e.g., `/usr/local/sbin/bn`), you may be running an outdated version. Verify with `bn --version`. The `bn-agent` script automatically prioritizes `~/.local/bin/bn`.
