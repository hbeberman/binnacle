...
/// Generate an entrypoint wrapper script for chaining parent and child entrypoints.
///
/// # Arguments
/// * `child_name` - Name of the child container
/// * `parent_entrypoint` - Path to parent's entrypoint script (e.g., "/entrypoint.sh")
/// * `child_entrypoint` - Path to child's entrypoint script
/// * `mode` - Chaining mode (Before or After)
///
/// # Returns
/// Shell script content that implements the chaining logic.
///
/// # Modes
/// - `Before`: Runs child logic, then execs parent entrypoint (child → parent)
/// - `After`: Runs parent logic, then runs child entrypoint (parent → child)
///
/// Note: Replace mode doesn't need a wrapper - child simply replaces parent.
pub fn generate_entrypoint_wrapper(
    child_name: &str,
    parent_entrypoint: &str,
    child_entrypoint: &str,
    mode: EntrypointMode,
) -> Result<String> {
    match mode {
        EntrypointMode::Replace => {
            // No wrapper needed for replace mode - child replaces parent
            Err(Error::Other(
                "generate_entrypoint_wrapper should not be called for Replace mode".to_string(),
            ))
        }
        EntrypointMode::Before => {
            // Child runs first, then execs into parent
            Ok(format!(
                r#"#!/bin/bash
# Entrypoint wrapper for container '{}' (before mode)
# Generated by binnacle - runs child logic then execs parent entrypoint

set -eu

# Run child entrypoint logic
echo "🔗 Running child entrypoint (before mode): {}"
if [ -f "{}" ] && [ -x "{}" ]; then
    source "{}"
else
    echo "⚠️  Warning: Child entrypoint '{}' not found or not executable"
fi

# Exec into parent entrypoint (replaces this process)
echo "🔗 Chaining to parent entrypoint: {}"
if [ -f "{}" ] && [ -x "{}" ]; then
    exec "{}" "$@"
else
    echo "❌ Error: Parent entrypoint '{}' not found or not executable"
    exit 1
fi
"#,
                child_name,
        ...
        EntrypointMode::After => {
            // Parent runs first, then child runs
            Ok(format!(
                r#"#!/bin/bash
# Entrypoint wrapper for container '{}' (after mode)
# Generated by binnacle - runs parent logic then child entrypoint

set -eu

# Run parent entrypoint logic (but don't exec - source it instead)
echo "🔗 Running parent entrypoint (after mode): {}"
if [ -f "{}" ] && [ -x "{}" ]; then
    source "{}"
else
    echo "❌ Error: Parent entrypoint '{}' not found or not executable"
...
echo "🔗 Chaining to child entrypoint: {}"
if [ -f "{}" ] && [ -x "{}" ]; then
    exec "{}" "$@"
else
    echo "⚠️  Warning: Child entrypoint '{}' not found or not executable"
...
///
/// # Returns
/// The entrypoint path that this container will use, considering:
/// - Custom entrypoint if defined in the definition (future enhancement)
/// - Parent's entrypoint if using before/after mode
/// - Default /entrypoint.sh otherwise
///
/// # Notes
...
"#,
    ...
    #[test]
    fn test_generate_entrypoint_wrapper_before_mode() {
        let wrapper = super::generate_entrypoint_wrapper(
            "rust-dev",
            "/parent-entrypoint.sh",
            "/child-entrypoint.sh",
            EntrypointMode::Before,
        )
        .unwrap();

        // Verify wrapper is a valid shell script
        assert!(wrapper.starts_with("#!/bin/bash"));
        assert!(wrapper.contains("before mode"));

        // Verify it sources child first
        assert!(wrapper.contains("source \"/child-entrypoint.sh\""));

        // Verify it execs parent
        assert!(wrapper.contains("exec \"/parent-entrypoint.sh\""));

        // Verify child comes before parent in the script
        let child_pos = wrapper.find("source \"/child-entrypoint.sh\"").unwrap();
        let parent_pos = wrapper.find("exec \"/parent-entrypoint.sh\"").unwrap();
        assert!(child_pos < parent_pos, "Child should run before parent");
    }

    #[test]
    fn test_generate_entrypoint_wrapper_after_mode() {
        let wrapper = super::generate_entrypoint_wrapper(
            "rust-dev",
            "/parent-entrypoint.sh",
            "/child-entrypoint.sh",
        ...
        // Verify wrapper is a valid shell script
        assert!(wrapper.starts_with("#!/bin/bash"));
        assert!(wrapper.contains("after mode"));

        // Verify it sources parent first
        assert!(wrapper.contains("source \"/parent-entrypoint.sh\""));

        // Verify it execs child
        assert!(wrapper.contains("exec \"/child-entrypoint.sh\""));

        // Verify parent comes before child in the script
        let parent_pos = wrapper.find("source \"/parent-entrypoint.sh\"").unwrap();
        let child_pos = wrapper.find("exec \"/child-entrypoint.sh\"").unwrap();
        assert!(parent_pos < child_pos, "Parent should run before child");
    }

    #[test]
    fn test_generate_entrypoint_wrapper_replace_mode_errors() {
        let result = super::generate_entrypoint_wrapper(
            "rust-dev",
            "/parent-entrypoint.sh",
            "/child-entrypoint.sh",
            EntrypointMode::Replace,
        ...
        // Replace mode should error - no wrapper needed
        assert!(result.is_err());
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("should not be called for Replace mode")
        );
    }
    ...
    #[test]
    fn test_entrypoint_wrapper_contains_error_handling() {
        let wrapper = super::generate_entrypoint_wrapper(
            "test",
            "/parent.sh",
            "/child.sh",
