...
**`bn graph lineage <id>`** - Walk **up** the ancestry chain to find the PRD, milestone, or parent task that explains why this task exists.
...
**`bn graph peers <id>`** - Find **sibling** tasks (tasks with the same parent) or **cousins** (tasks sharing a grandparent).
...
**`bn graph descendants <id>`** - Walk **down** to find child tasks and subtasks.
...
pub fn goodbye(repo_path: &Path, reason: Option<String>, force: bool) -> Result<GoodbyeResult> {
    ...
    // - BN_MCP_SESSION mode: should_terminate is true (agent should self-terminate based on signal)
    let should_terminate = if env_agent_id.is_some() {
        // Container mode: external system manages lifecycle, don't signal termination
        false
    } else if mcp_session_id.is_some() {
        // MCP mode: agent should self-terminate (but bn won't kill the process)
        true
    } else {
